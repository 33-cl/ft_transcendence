...
export async function processImageSecurely(buffer: Buffer, mimeType: string): Promise<Buffer>
{
  try {
    ...
    switch (mimeType)
    {
      ...
      case 'image/png':
        return await sharpPipeline.png({ quality: AVATAR_CONFIG.QUALITY, progressive: true }).toBuffer();
      case 'image/webp':
        return await sharpPipeline.webp({ quality: AVATAR_CONFIG.QUALITY }).toBuffer();
      default:
