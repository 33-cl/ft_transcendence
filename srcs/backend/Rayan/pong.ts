...
export class PongGame {
    ...
    movePaddle(player: 'left' | 'right', direction: 'up' | 'down') {
        const speed = this.state.paddleSpeed;
        if (player === 'left') {
            if (direction === 'up') this.state.leftPaddleY = Math.max(0, this.state.leftPaddleY - speed);
            else this.state.leftPaddleY = Math.min(this.state.canvasHeight - this.state.paddleHeight, this.state.leftPaddleY + speed);
        } else {
            if (direction === 'up') this.state.rightPaddleY = Math.max(0, this.state.rightPaddleY - speed);
            else this.state.rightPaddleY = Math.min(this.state.canvasHeight - this.state.paddleHeight, this.state.rightPaddleY + speed);
        }
    }
    ...
    update() {
        ...
        // Paddle gauche
        if (
            this.state.ballX - this.state.ballRadius < this.state.paddleMargin + this.state.paddleWidth &&
        ...
        // Paddle droit
        if (
            this.state.ballX + this.state.ballRadius > this.state.canvasWidth - this.state.paddleMargin - this.state.paddleWidth &&
