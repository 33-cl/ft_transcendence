...
export class PongGame {
        ...
        if (this.state.paddles && this.state.paddles.length === 2) {
            if (player === 'left') {
                if (direction === 'up')
                    ...
                    this.state.paddles[0].y = Math.min(this.state.canvasHeight - this.state.paddleHeight, this.state.paddles[0].y + speed);
            }
            else {
                if (direction === 'up')
                    this.state.paddles[1].y = Math.max(0, this.state.paddles[1].y - speed);
                else
                    this.state.paddles[1].y = Math.min(this.state.canvasHeight - this.state.paddleHeight, this.state.paddles[1].y + speed);
            }
        }
            ...
            if (idx !== -1) {
                ...
                if (player === 'A' || player === 'C') {
                    if (direction === 'up')
                        ...
                        this.state.paddles[idx].y = Math.min(this.state.canvasHeight - this.state.paddleHeight, this.state.paddles[idx].y + speed);
                }
                // Paddle B : horizontal (x bouge, stocké dans y)
                    ...
                    if (direction === 'up')
                        this.state.paddles[idx].y = Math.max(minX, this.state.paddles[idx].y - speed);
                    else
                        this.state.paddles[idx].y = Math.min(maxX, this.state.paddles[idx].y + speed);
                }
            }
        }
    }
    update() {
        ...
        if (this.state.paddles && this.state.paddles.length === 3) {
            ...
            // --- Paddle A (gauche) ---
            if (this.state.ballX - ballRadius < paddleMargin + paddleWidth &&
            ...
            // --- Paddle C (droit) ---
            if (this.state.ballX + ballRadius > canvasWidth - paddleMargin - paddleWidth &&
            ...
            if (this.state.ballY + ballRadius > paddleB_Y &&
