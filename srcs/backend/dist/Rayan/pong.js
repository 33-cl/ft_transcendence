...
export class PongGame {
    constructor(numPlayers = 2, onGameEnd) {
        this.interval = null;
        this.ballStartTime = 0;
        this.ballDelayMs = 3000; // 3 secondes
        this.isFirstLaunch = true; // Track si c'est le premier lancement
    ...
    movePaddle(player, direction) {
        ...
        if (this.state.paddles && this.state.paddles.length === 2) {
            if (player === 'left' || player === 'A') {
                ...
                if (direction === 'up')
                    this.state.paddles[0].y = Math.max(0, this.state.paddles[0].y - speed);
                else
                    this.state.paddles[0].y = Math.min(this.state.canvasHeight - this.state.paddles[0].height, this.state.paddles[0].y + speed);
            }
            else if (player === 'right' || player === 'C') {
                // Paddle C est à l'index 1
                if (direction === 'up')
                    this.state.paddles[1].y = Math.max(0, this.state.paddles[1].y - speed);
                else
                    this.state.paddles[1].y = Math.min(this.state.canvasHeight - this.state.paddles[1].height, this.state.paddles[1].y + speed);
            }
        }
        // Mode 1v1v1 : paddles[0]=A, paddles[1]=B, paddles[2]=C
            ...
            if (idx !== -1) {
                ...
                if (player === 'A' || player === 'C') {
                    if (direction === 'up')
                        this.state.paddles[idx].y = Math.max(0, this.state.paddles[idx].y - speed);
                    else
                        this.state.paddles[idx].y = Math.min(this.state.canvasHeight - this.state.paddles[idx].height, this.state.paddles[idx].y + speed);
                }
                // Paddle B : horizontal (x bouge)
                    ...
                    if (direction === 'up')
                        this.state.paddles[idx].x = Math.max(minX, this.state.paddles[idx].x - speed); // left
                    else
                        this.state.paddles[idx].x = Math.min(maxX, this.state.paddles[idx].x + speed); // right
                }
            }
        }
    }
    update() {
        ...
        if (this.state.paddles && this.state.paddles.length === 3) {
            ...
            // --- Paddle A (gauche) ---
            if (this.state.ballX - ballRadius < paddles[0].x + paddles[0].width &&
            ...
            // --- Paddle C (droit) ---
            if (this.state.ballX + ballRadius > paddles[2].x &&
            ...
            if (this.state.ballY + ballRadius > paddles[1].y &&
                ...
                if (p.score >= this.state.win) {
                    ...
                    if (this.onGameEnd) {
                        ...
                        if (winner && losers.length > 0) {
                            ...
                            this.onGameEnd({ side: winner.side, score: winner.score }, { side: loser.side, score: loser.score });
                        }
                    }
                }
            }
        ...
        if (this.state.paddles && this.state.paddles.length === 2) {
            ...
            if (this.state.ballX + ballRadius > paddles[1].x &&
