...
function handleGameTick(io) {
    for (const [roomName, room] of Object.entries(rooms)) {
        ...
        if (typedRoom.pongGame && typedRoom.pongGame.state.running) {
            ...
            if (typedRoom.pongGame.state.paddles && typedRoom.pongGame.state.paddles.some((p) => p.score > 0)) {
                console.log(`[DEBUG SCORES] room=${roomName} scores=`, typedRoom.pongGame.state.paddles.map((p) => p.score));
            }
        }
    ...
    if ((message.type === 'keydown' || message.type === 'keyup') && room.pongGame && room.paddleBySocket) {
        ...
        if (room.isLocalGame) {
            ...
            if (player === 'left')
                ...
                mappedPlayer = 'C'; // Changé de B vers C
