...
const STAR_SIZE_RANGE = [0.5, 3];
const STAR_OPACITY_RANGE = [0.1, 2];
...
const SHOOTING_STAR_SPEED_RANGE = [3, 8];
const SHOOTING_STAR_LENGTH_RANGE = [50, 150];
...
function calculateStarCount(canvasWidth, canvasHeight) {
...
(function (CollapseState) {
    CollapseState[CollapseState["NORMAL"] = 0] = "NORMAL";
    CollapseState[CollapseState["RING_COLLAPSE"] = 1] = "RING_COLLAPSE";
    CollapseState[CollapseState["STARS_COLLAPSE"] = 2] = "STARS_COLLAPSE";
    CollapseState[CollapseState["SHOOTING_STARS_COLLAPSE"] = 3] = "SHOOTING_STARS_COLLAPSE";
    CollapseState[CollapseState["FINAL_COLLAPSE"] = 4] = "FINAL_COLLAPSE";
    CollapseState[CollapseState["RESETTING"] = 5] = "RESETTING";
    CollapseState[CollapseState["RESET_COMPLETE"] = 6] = "RESET_COMPLETE"; // État final - plus d'actions possibles
...
class BlackHole {
    constructor(centerX, centerY) {
        ...
        this.centerY = centerY;
        this.ringStars = [];
        this.collapseState = CollapseState.NORMAL;
        this.collapseTimer = 0;
        this.isVisible = true;
        this.blackHoleOpacity = 1;
        this.finalBlackHoleX = 0; // Position finale du trou noir pour le reset
        this.finalBlackHoleY = 0;
        this.angle = Math.random() * Math.PI * 2;
    ...
    checkShootingStarCollision(shootingStar) {
    ...
    draw(ctx) {
        ...
        if (!this.isVisible || this.collapseState === CollapseState.RESETTING || this.collapseState === CollapseState.RESET_COMPLETE)
            return;
        // Dessiner les traînées des étoiles de l'anneau d'abord
        this.ringStars.forEach(star => {
            if (star.trail.length < 2 || star.opacity <= 0)
                return;
            // Dessiner la traînée
            for (let i = 1; i < star.trail.length; i++) {
                const point = star.trail[i];
                const prevPoint = star.trail[i - 1];
                if (!point || !prevPoint || point.opacity <= 0)
                    continue;
...
class ShootingStar {
    ...
    startCollapseToBlackHole(_blackHoleX, _blackHoleY) {
        this.isCollapsingToBlackHole = true;
    }
    update(canvasWidth, canvasHeight, mouseX, mouseY, blackHoleX, blackHoleY) {
        this.age++;
        if (this.isCollapsingToBlackHole && blackHoleX !== undefined && blackHoleY !== undefined) {
            // Mouvement vers le trou noir
            const dx = blackHoleX - this.x;
...
class BackgroundStarfield {
    constructor(canvasId) {
        ...
        this.hasResetStarted = false; // Pour s'assurer que le reset ne se lance qu'une fois
        this.currentStarCount = 0; // Suivre le nombre actuel d'étoiles
    ...
    createStars() {
        ...
        this.stars = Array.from({ length: this.currentStarCount }, () => new Star(centerX, centerY));
    ...
    updateShootingStars() {
        this.shootingStars.forEach(shootingStar => {
            ...
            shootingStar.update(this.canvas.width, this.canvas.height, mouseX, mouseY, this.blackHole?.x, this.blackHole?.y);
        });
...
export function setStarsHoverColor(color) {
    currentHoverColor = color;
}
export function getColorRgb(difficulty) {
    switch (difficulty) {
        case 'easy':
