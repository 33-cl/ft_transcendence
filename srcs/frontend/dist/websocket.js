...
function joinOrCreate1v1Room() {
    return __awaiter(this, void 0, void 0, function* () {
        if (joinInProgress) {
            if (lastJoinPromise)
                yield lastJoinPromise;
            else
                return;
        }
        joinInProgress = true;
        ...
        lastJoinPromise = (() => __awaiter(this, void 0, void 0, function* () {
            try {
                // Synchronise la room réelle côté backend en cherchant le socket.id dans toutes les rooms
                const resRooms = yield fetch('http://localhost:8080/rooms');
                const dataRooms = yield resRooms.json();
                ...
                // Si déjà dans une room 1v1 non pleine, ne rien faire (FRONTEND GUARD)
                if (myRoom) {
                    const room = dataRooms.rooms[myRoom];
                    if (room.maxPlayers === 2 && room.players.length < 2) {
                        currentRoom = myRoom;
                        console.log('[FRONTEND GUARD] Déjà dans une room 1v1 non pleine (backend):', myRoom);
                        joinInProgress = false;
                        lastJoinPromise = null;
                        return;
                    }
                }
                // 1. Cherche une room 1v1 non pleine
                let roomName = null;
                for (const [name, room] of Object.entries(dataRooms.rooms)) {
                    const r = room;
                ...
                if (!roomName) {
                    const res2 = yield fetch('http://localhost:8080/rooms', {
                        method: 'POST',
                    ...
                    for (let i = 0; i < 5; i++) { // 5 tentatives max
                        const check = yield fetch('http://localhost:8080/rooms');
                        const checkData = yield check.json();
                        if (checkData.rooms[roomName]) {
                            found = true;
                            break;
                        }
                        yield new Promise(r => setTimeout(r, 30)); // attend 30ms
                    }
                    if (!found) {
...
window.joinOrCreate1v1Room = joinOrCreate1v1Room;
// Fonction générique pour rejoindre ou créer automatiquement une room de n joueurs
function joinOrCreateRoom(maxPlayers) {
    return __awaiter(this, void 0, void 0, function* () {
        if (joinInProgress) {
            if (lastJoinPromise)
                yield lastJoinPromise;
