...
export function initPongRenderer(canvasId = 'map') {
...
export function resetPongRenderer() {
    if (canvas && ctx) {
    ...
    if (scoreElem) {
        if (gameState.paddles && Array.isArray(gameState.paddles)) {
            if (gameState.paddles.length === 4) {
                ...
                scoreElem.innerHTML = gameState.paddles.map((p, i) => `<span id='score${i}' style='color: ${getColorForSide(p.side)}'>${p.side}: ${p.score || 0}</span>`).join(' | ');
            }
...
window.draw = draw;
//# sourceMappingURL=pongRenderer.js.map